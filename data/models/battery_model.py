...
SAMPLE_INTERVAL_S = 5.0  # poll interval — approximation
...
class BatteryEstimator:
    ...
    def __init__(self):
        self._soc: Dict[str, float] = {}  # driver_id -> kJ remaining
    ...
    def update(self, driver_id: str, throttle: int, brake: int, speed: int,
        ...
        """
        Update SOC estimate for a driver based on current telemetry.

        Returns estimated SOC as percentage (0-100).
        """
        if driver_id not in self._initialized:
            self._soc[driver_id] = BATTERY_CAPACITY_KJ * 0.5  # assume 50% at start
            self._initialized[driver_id] = True

        current_kj = self._soc[driver_id]

        # Deploy: proportional to throttle %, reduced above 290 km/h
        if throttle > 10:
            ...
            deploy_kw = MGUK_MAX_DEPLOY_KW * throttle_frac * speed_factor * boost_factor * 0.4
            current_kj -= deploy_kw * dt

        # Harvest: proportional to braking intensity
        if brake > 10:
            brake_frac = min(brake / 100.0, 1.0)
            harvest_kw = MGUK_MAX_HARVEST_KW * brake_frac * 0.6
